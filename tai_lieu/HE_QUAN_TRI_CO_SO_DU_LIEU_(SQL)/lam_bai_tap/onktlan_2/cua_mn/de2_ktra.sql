...
SELECT * FROM CAU2
--cau3
create function ham(@hotenkhach nchar(10), @x int, @y int)
returns @bang table(
	mahang int,
	tenhang nchar(10),
	soluong int,
...
begin
	insert into @bang 
		...
		where hotenkhach=@hotenkhach and ngayban between @x and @y
	return 
end

select *from ham('lan','1','4')
...
begin
	...
	set @tong= (select sum(soluong)  
				from hdban inner join hangban on hdban.mahd=hangban.mahd inner join hang on hangban.mahang=hang.mahang
				where tenhang=@tenhang and ngayban=@x)

	return @tong
end
drop function hamm
select dbo.hamm('h2','7') as N'tongSL'
----------------3
